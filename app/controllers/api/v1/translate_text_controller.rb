...
class Api::V1::TranslateTextController < ApplicationController
  def create
    ...
    system_prompt = TranslationPrompt.new(
      from_language: translate_params[:from_language],
      to_language: translate_params[:to_language]
    )

    raise CustomException.new(system_prompt.errors.full_messages.to_sentence) unless system_prompt.valid?
